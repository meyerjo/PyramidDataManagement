...
class FileCommentView():
    ...
    @view_config(route_name='files_comment', permission='authenticatedusers', renderer='template/index.pt',
                     request_method='POST', request_param='description')
    ...
    @view_config(route_name='files_comment', permission='authenticatedusers', renderer='template/index.pt',
                     request_method='POST', request_param='label')
    def add_label(self):
        log = logging.getLogger(__name__)
        file = self.request.matchdict['file']
        action = self.request.matchdict['action']
    ...
    @view_config(route_name='files_comment', permission='authenticatedusers', renderer='template/index.pt',
                 request_method='POST')
    def comments(self):
        # this is the most general case, we get a post request which doesn't match any of the more general view_config
        # thus this is the request which is executed, we write the POST array to the log and refer the request
        # back to the normal view
        log = logging.getLogger(__name__)

        log.warning('Not yet implemented {0}'.format(self.request.POST))
